...
RUN apt-get update && \
    ...
    docker-php-ext-install -j$(nproc) xdebug opcache mysql pdo_mysql intl mbstring bz2 gd exif sockets sysvsem sysvshm sysvmsg wddx shmop calendar dom xsl soap xmlrpc && \
